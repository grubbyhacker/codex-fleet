...
const setprivPath = process.env.PATH?.split(delimiter)
  .map((directory) => join(directory, "setpriv"))
  .find(existsSync);
const runnable = process.platform === "linux" && process.getuid?.() === 0 && Boolean(setprivPath);

describe.skipIf(!runnable)(
  "Linux test-only per-session UID/GID isolation proof (requires root and setpriv)",
  () => {
