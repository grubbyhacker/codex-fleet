...
describe("codex worker backend", () => {
  ...
  it("maps explicit GPT-5.6 routes to concrete Codex model ids", () => {
    expect(resolveCodexWorkerConfig("standard", "gpt-5.6-luna")).toMatchObject({
      model: "gpt-5.6-luna"
    });
    expect(resolveCodexWorkerConfig("standard", "gpt-5.6-terra")).toMatchObject({
      model: "gpt-5.6-terra"
    });
